<template>
  <div class="app-container">
    ...
    <pagination v-show="total>0" :total="total" :page.sync="listQuery.page" :limit.sync="listQuery.limit" @pagination="retrieveReadWhitelist" />
...
<script>
import WriteWhitelistData from './services/readWhitelistData'
import waves from '@/components/Waves' // Waves directive
import Pagination from '@/components/Pagination' // secondary package based on el-pagination

export default {
  name: 'ReadWhitelistList',
  components: {
    Pagination
  },
  directives: { waves },
  ...
  created() {
    setInterval(function() {
      this.retrieveReadWhitelist()
    }.bind(this), 1000)
  },
  methods: {
    retrieveReadWhitelist() {
      WriteWhitelistData.getAll()
        .then(response => {
          // Choose the order of the rows in the table
          if (this.listQuery.sort === '+id') {
            this.readwhitelist = response.data.sort((a, b) => (a.ip_src > b.ip_src ? 1 : -1))

            // Search for substrings
            if (typeof this.listQuery.ip_src !== 'undefined') {
              this.readwhitelist = this.filter(this.readwhitelist, this.listQuery.ip_src, 'ip_src')
            }
            if (typeof this.listQuery.ip_dst !== 'undefined') {
              this.readwhitelist = this.filter(this.readwhitelist, this.listQuery.ip_dst, 'ip_dst')
            }
            if (typeof this.listQuery.active !== 'undefined') {
              this.readwhitelist = this.filter(this.readwhitelist, this.listQuery.active, 'active')
            }

            // The total number of element in the array
            this.total = this.readwhitelist.length
            ...
            this.readwhitelist = this.readwhitelist.slice(init_a, finish_a)
          } else {
            this.readwhitelist = response.data.sort((a, b) => (a.ip_src < b.ip_dst ? 1 : -1))
    ...
    getSortClass: function(key) {
      const sort = this.retrieveReadWhitelist().sort
