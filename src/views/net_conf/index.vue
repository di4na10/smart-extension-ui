<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.parameter" placeholder="Parameter" style="width: 150px;" class="filter-item" @change="handleFilter" />
    ...
    >
      <el-table-column label="Parameter" prop="parameter" sortable="custom" align="center" width="250px">
    ...
    <pagination v-show="total>0" :total="total" :page.sync="listQuery.page" :limit.sync="listQuery.limit" @pagination="retrieveNetConf" />
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="100px" style="width: 80%; margin-left:50px;">
        <el-form-item label="Parameter" prop="parameter">
          <el-input v-model="temp.parameter" />
        </el-form-item>
...
<script>
import NetConfData from './services/netConfData'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        parameter: [{ required: true, message: 'Parameter is required' }],
  ...
  methods: {
      ...
      NetConfData.getAll()
        .then(response => {
          ...
          if (this.listQuery.sort === '+id') {
            this.netconf = response.data.sort((a, b) => (a.Name > b.Name ? 1 : -1))

            // Search for substrings
            if (typeof this.listQuery.parameter !== 'undefined') {
              this.netconf = this.filter(this.netconf, this.listQuery.parameter, 'parameter')
            }
            if (typeof this.listQuery.value !== 'undefined') {
              this.netconf = this.filter(this.netconf, this.listQuery.value, 'value')
            }

            // The total number of element in the array
            this.total = this.netconf.length
            ...
            this.netconf = this.netconf.slice(init_a, finish_a)
          } else {
            this.netconf = response.data.sort((a, b) => (a.parameter < b.parameter ? 1 : -1))
    ...
    handleDelete(row) {
      ...
      NetConfData.delete(this.temp.parameter)
        .then(() => {
          this.$notify({
            title: 'Success',
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          NetConfData.update(tempData.parameter, tempData)
            .then(() => {
    ...
    getSortClass: function(key) {
      const sort = this.retrieveNetConf().sort
