<template>
  <div class="app-container">
    ...
    <pagination v-show="total>0" :total="total" :page.sync="listQuery.page" :limit.sync="listQuery.limit" @pagination="retrieveWriteWhitelist" />
...
<script>
import WriteWhitelistData from './services/writeWhitelistData'
import waves from '@/components/Waves' // Waves directive
import Pagination from '@/components/Pagination' // secondary package based on el-pagination

export default {
  name: 'WriteWhitelistList',
  components: {
    Pagination
  },
  directives: { waves },
  ...
  created() {
    setInterval(function() {
      this.retrieveWriteWhitelist()
    }.bind(this), 1000)
  },
  methods: {
    retrieveWriteWhitelist() {
      WriteWhitelistData.getAll()
        .then(response => {
          // Choose the order of the rows in the table
          if (this.listQuery.sort === '+id') {
            this.writewhitelist = response.data.sort((a, b) => (a.ip_src > b.ip_src ? 1 : -1))

            // Search for substrings
            if (typeof this.listQuery.ip_src !== 'undefined') {
              this.writewhitelist = this.filter(this.writewhitelist, this.listQuery.ip_src, 'ip_src')
            }
            if (typeof this.listQuery.ip_dst !== 'undefined') {
              this.writewhitelist = this.filter(this.writewhitelist, this.listQuery.ip_dst, 'ip_dst')
            }
            if (typeof this.listQuery.active !== 'undefined') {
              this.writewhitelist = this.filter(this.writewhitelist, this.listQuery.active, 'active')
            }

            // The total number of element in the array
            this.total = this.writewhitelist.length
            ...
            this.writewhitelist = this.writewhitelist.slice(init_a, finish_a)
          } else {
            this.writewhitelist = response.data.sort((a, b) => (a.ip_src < b.ip_dst ? 1 : -1))
    ...
    getSortClass: function(key) {
      const sort = this.retrieveWriteWhitelist().sort
