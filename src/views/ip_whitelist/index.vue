<template>
  <div class="app-container">
    ...
    <pagination v-show="total>0" :total="total" :page.sync="listQuery.page" :limit.sync="listQuery.limit" @pagination="retrieveIPWhitelist" />
...
<script>
import IPWhitelistData from './services/ipwhitelistData'
import waves from '@/components/Waves' // Waves directive
import Pagination from '@/components/Pagination' // secondary package based on el-pagination

export default {
  name: 'IPWhitelistList',
  components: {
    Pagination
  },
  directives: { waves },
  ...
  created() {
    setInterval(function() {
      this.retrieveIPWhitelist()
    }.bind(this), 1000)
  },
  methods: {
    retrieveIPWhitelist() {
      IPWhitelistData.getAll()
        .then(response => {
          // Choose the order of the rows in the table
          if (this.listQuery.sort === '+id') {
            this.ipwhitelist = response.data.sort((a, b) => (a.IP > b.IP ? 1 : -1))

            // Search for substrings
            if (typeof this.listQuery.IP !== 'undefined') {
              this.ipwhitelist = this.filter(this.ipwhitelist, this.listQuery.IP, 'IP')
            }

            // The total number of element in the array
            this.total = this.ipwhitelist.length
            ...
            this.ipwhitelist = this.ipwhitelist.slice(init_a, finish_a)
          } else {
            this.ipwhitelist = response.data.sort((a, b) => (a.IP < b.IP ? 1 : -1))
    ...
    handleDelete(row, id) {
      ...
      IPWhitelistData.delete(this.temp.IP)
        .then(() => {
          this.$notify({
            title: 'Success',
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          IPWhitelistData.update(tempData.IP, tempData)
            .then(() => {
    ...
    getSortClass: function(key) {
      const sort = this.retrieveIPWhitelist().sort
